...
public class PerNivelRemunerativoFacadeImpl implements PerNivelRemunerativoFacade{
    ...
    public List<PerNivelRemunerativo> load(int first, int pageSize, String sortField, Map<String, Object> filters) {
        ...
        String sql;
        ql.add(em.createQuery("SELECT o " + (sql="FROM PerNivelRemunerativo o WHERE 1=1 "
    ...
    @Override
    public void edit(PerNivelRemunerativo entity) {
        if (entity.getIdNivelR() == null) {
            em.persist(entity);
        } else {
            em.merge(entity);
        }
